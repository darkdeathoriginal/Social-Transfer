...
global.conn = new TelegramClient(stringSession, apiId, apiHash, {
  ...
  conn.addEventHandler(async (event) => {
    for (const module of modules) {
      await module.handleEvent(event, conn);
    }
  }, new NewMessage({}));
  ...
  if(session ==""){
    let a = conn.session.save()
    let file = await fs.readFileSync(".env","utf8")
    file+=`\nSESSION=${a}`
    fs.writeFileSync(".env",file)
  }
  console.log('Bot is ready.');
  await conn.sendMessage('me', { message: 'Bot has been started..' });
