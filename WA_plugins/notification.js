  ...
  try {
    ...
    while (true) {
      ...
      for (let i of cources) {
        ...
        if(!state[i.id]["announcement"]) state[i.id]["announcement"] = announcements[0]?.id;
        if(!state[i.id]["courseWork"]) state[i.id]["courseWork"] = courseWorks[0]?.id;
        if(!state[i.id]["courseWorkMaterial"]) state[i.id]["courseWorkMaterial"] = courseWorkMaterials[0]?.id;
